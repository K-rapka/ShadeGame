...
export function Grid({score,setScore,setScreen}:IGrid):JSX.Element{
    ...
    const testGrid:JSX.Element[]= grid.map((item,key)=>{
        const row = item.map((item,key) =>{
            return <Circle key={key} imposter = {item}step={step}  setStep={setStep} setLevel={setLevel} setScreen={setScreen}/>
        })
